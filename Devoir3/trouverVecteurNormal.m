function [vecteurNormalCylindre, vecteurNormalBalle] = trouverVecteurNormal(rCMBalle, rCMBoite, thetaBoite, typeCollision)
    ...
    positionCMBalleModif = transpose(matriceRotationY(thetaBoite)*transpose(rCMBalle));
    positionCMBoiteModif = transpose(matriceRotationY(thetaBoite)*transpose(rCMBoite));
    
 %CAS 1: La balle entre en collision avec le cercle en haut ou en bas du cylindre
    if(typeCollision==Constantes.COLLISION_BASE)
      if (positionCMBalleModif(3)>=positionCMBoiteModif(3) + Constantes.HAUTEUR_BOITE_m/2)
        %collision avec la base circulaire de haut
        pointCollision = positionCMBalleModif-[0,0,Constantes.RAYON_BALLE_m];
      else
        %collision avec la base circulaire du bas
        pointCollision = positionCMBalleModif+[0,0,Constantes.RAYON_BALLE_m];
      endif
      vecteurNormalBalle = positionCMBalleModif-pointCollision;
